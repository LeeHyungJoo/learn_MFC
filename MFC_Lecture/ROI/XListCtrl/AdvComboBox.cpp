...
void CAdvComboBox::LoadString( UINT nStringID )
{
	...
	{
		nIDS = static_cast<UINT>(GetWindowLongPtr( GetSafeHwnd(), GWL_ID ));
	}
	...
	_tcscpy_s(szTok, sizeof(szTok), (LPCTSTR)strItems);

	token = _tcstok_s(szTok, seps, &next_token);
	while( token != NULL )
	{
		AddString( token );
...
int CAdvComboBox::GetLBText(int nIndex, LPTSTR lpszText)
{
	...
	_tcscpy_s( lpszText, sizeof(lpszText),m_iter->strText.c_str() );
	return static_cast<int>(m_iter->strText.length()+1);
}
...
int CAdvComboBox::GetText(LPTSTR lpszText)
{
	...
	{	
		...
		_tcscpy_s(lpszText, sizeof(lpszText), (LPCTSTR)str);
		return str.GetLength();
	}
	else
	{
		_tcscpy_s(lpszText, sizeof(lpszText), m_strEdit.c_str());
		return static_cast<int>(m_strEdit.length());
	}
}
...
int CAdvComboBox::SetItemData(int nIndex, DWORD_PTR dwItemData)
{
	m_iter = m_list.begin();
	advance( m_iter, nIndex );
	if( m_iter == m_list.end() || nIndex > (int)m_list.size() )
...
DWORD_PTR CAdvComboBox::GetItemData(int nIndex)
{
	m_iter = m_list.begin();
	advance( m_iter, nIndex );
	if( m_iter == m_list.end() || nIndex > (int)m_list.size() )
	...
	return (DWORD_PTR)m_iter->vpItemData;
}
...
void CAdvComboBox::SelNextPage()
{
	...
	else
	{
		...
		int nSize = static_cast<int>(m_list.size());
		m_iter = m_list.begin();
...
//			m_nCurSel = -1;
			if( m_dwACBStyle & ACBS_AUTOAPPEND && m_bAutoAppend )
			{
				...
				int nEditLen = static_cast<int>(str.length());
				if( !nEditLen )
					return;
				int nStartSel;
				int nEndSel;
				...
				{
					item = *m_iter;
					int nPos = static_cast<int>(m_iter->strText.find( str, 0 ));
					if( nPos == 0 )
					{
						bAutoAppendInProgress = true;
						m_pEdit->SetWindowText( m_iter->strText.c_str() );
						m_pEdit->SetSel( nEditLen, static_cast<int>(m_iter->strText.length()), TRUE );
...
void CAdvComboBox::OnActivateApp(BOOL bActive, HTASK hTask) 
{
	...
	CWnd::OnActivateApp(bActive, (DWORD)(DWORD_PTR)hTask);
}
