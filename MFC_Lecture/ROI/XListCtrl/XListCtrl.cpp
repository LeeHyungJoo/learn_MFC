...
BEGIN_MESSAGE_MAP(CXListCtrl, CListCtrl)
	...
	ON_REGISTERED_MESSAGE(WM_CHECKCOMBO_ESCAPE, OnCheckComboEscape)
	ON_REGISTERED_MESSAGE(WM_CHECKCOMBO_COMPLETE, OnCheckComboComplete)
#endif
							  ...
							  XLISTCTRLDATA *pXLCD)
{
	...
	if (pXLCD[nSubItem].nCheckedState == 1)
	{
		...
		// draw the checkmark
		int x = chkboxrect.left + (chkboxrect.Width() / 4) * 3;
		ASSERT(x < chkboxrect.right);
		int y = chkboxrect.top + chkboxrect.Height() / 4;
		int i;
		for (i = 0; i < 7; i++)
		{
			pDC->MoveTo(x, y);
						  ...
						  XLISTCTRLDATA *pXLCD)
{
	...
	if (!str.IsEmpty())
	{
		...
		// check if strikeout specified for subitem
		if (pXLCD && pXLCD[nSubItem].bStrikeOut)
		{
			CFont *font = pDC->GetCurrentFont();
			if (font)
			{
				...
				font->GetLogFont(&lf);
				lf.lfStrikeOut = 1;
				strikeoutfont.CreateFontIndirect(&lf);
				pOldFont = pDC->SelectObject(&strikeoutfont);
			}
		}
		pDC->SetBkMode(TRANSPARENT);
		pDC->SetTextColor(crText);
		...
		else
		{
			if (pXLCD[nSubItem].nCheckedState >= 0)
			{
				...
				if (pWnd && ::IsWindow(pWnd->m_hWnd))
					pWnd->SendMessage(WM_XLISTCTRL_CHECKBOX_CLICKED, (WPARAM)this, nItem);
...
{
	////XLISTCTRL_TRACE(_T("in CXListCtrl::OnLButtonDblClk\n"));
	...
	else
	{
		...
		else
		{
			...
			else if (pXLCD[nSubItem].bCheckCombo)
			{
				DrawCheckComboBox(nItem, nSubItem);
			}
			else if (pXLCD[nSubItem].bEdit)
			{
				//CListCtrl::OnLButtonDblClk(nFlags, point);
				DrawEdit(nItem, nSubItem, pXLCD[nSubItem].dwStyle);
			}
		}
	}
...
///////////////////////////////////////////////////////////////////////////////
// SetEdit
BOOL CXListCtrl::SetEdit(int nItem, int nSubItem, bool bEnable, DWORD dwStyle)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	pXLCD[nSubItem].bEdit = bEnable;
	pXLCD[nSubItem].dwStyle = dwStyle;
...
BOOL CXListCtrl::SetStrikeOut(int nItem, int nSubItem, BOOL bStrikeOut)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	// update bold flag
	pXLCD[nSubItem].bStrikeOut = bStrikeOut;
...
BOOL CXListCtrl::GetStrikeOut(int nItem, int nSubItem)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	// return bold flag
	return pXLCD[nSubItem].bStrikeOut;
}
...
void CXListCtrl::SetItemColors(int nItem, COLORREF crText, COLORREF crBackground)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	if (pXLCD)
	{
		...
		{
			pXLCD[column].crText = crText;
			pXLCD[column].crBackground = crBackground;
		}
	}
}
...
BOOL CXListCtrl::SetCheckComboBox(int nItem,
								  int nSubItem,
								  BOOL bEnableCombo,
								  CStringArray *psa,		// should not be allocated on stack
								  BOOL bSort /*= FALSE*/)
{
	...
	// update flag
	pXLCD[nSubItem].bCheckCombo = bEnableCombo;
...
//    5 - used to close check combo listbox, set in OnCheckComboEscape (user hits Escape
//        or listbox loses focus)
//    6 - used to get check combo listbox selection, then close check combo listbox,
//        set in OnCheckComboReturn and OnCheckComboLButtonUp (user hits Enter
//        or clicks on item in listbox)
void CXListCtrl::OnTimer(UINT_PTR nIDEvent)
{
			...
			if (i != LB_ERR)
			{
					...
					(m_nComboSubItem >= 0 && m_nComboSubItem < GetColumns()))
				{
					...
					if (!pWnd)
						pWnd = GetOwner();
					if (pWnd && ::IsWindow(pWnd->m_hWnd))
						pWnd->SendMessage(WM_XLISTCTRL_COMBO_SELECTION, WPARAM(this), m_nComboItem);
				}
			}
	...
	else if (nIDEvent == 5)		// close check combo listbox
	{
		KillTimer(nIDEvent);
		...
		{
			UpdateSubItem(m_nComboItem, m_nComboSubItem);
			m_pCheckCombo->DestroyWindow();
			delete m_pCheckCombo;
		}
		m_pCheckCombo = NULL;
	}
	else if (nIDEvent == 6)		// get check combo listbox selection, then close check combo listbox
	{
		KillTimer(nIDEvent);
		...
		if (m_pCheckCombo)
		{
			CString str = m_pCheckCombo->GetText();
...
LRESULT CXListCtrl::OnCheckComboEscape(WPARAM, LPARAM)
{
	SetTimer(5, 50, NULL);
	return 0;
}

LRESULT CXListCtrl::OnCheckComboComplete(WPARAM, LPARAM)
{
	SetTimer(6, 50, NULL);
	return 0;
}
...
void CXListCtrl::DrawCheckComboBox(int nItem, int nSubItem)
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
	...
	m_pCheckCombo = new ChkCombo();
	ASSERT(m_pCheckCombo);
	...
	if (m_pCheckCombo)
	{
		...
		DWORD dwStyle = WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_TILED;
		BOOL bSuccess = m_pCheckCombo->Create(dwStyle,
											  m_rectComboList,
											  this,
											  0);
		...
		if (bSuccess)
		{
			...
			//m_pCheckCombo->SetDefaultVisibleItems(pXLCD[nSubItem].nComboListHeight);
			...
			if (pXLCD[nSubItem].psa)
			{
				try
				{
					...
					int pos = 0;
					CString token = m_strInitialString.Tokenize(_T(", "), pos);
					while (!token.IsEmpty())
					{
						int index = m_pCheckCombo->FindString(0, token);

						if (index != CB_ERR)
							m_pCheckCombo->SetCheck(index, TRUE);
						token = m_strInitialString.Tokenize(_T(", "), pos);
					}
		...
		//Combo rect
		m_pCheckCombo->GetClientRect(&rctComboBox);
		//DropDownList rect
		m_pCheckCombo->GetDroppedControlRect(&rctDropDown);

		//Get Item height
		int itemHeight = m_pCheckCombo->GetItemHeight(-1);
		//Converts coordinates
		m_pCheckCombo->GetParent()->ScreenToClient(&rctDropDown);
		//Set height
		rctDropDown.bottom = rctDropDown.top + rctComboBox.Height() + itemHeight * 5;
		//apply changes
		m_pCheckCombo->MoveWindow(&rctDropDown);

		m_pCheckCombo->Invalidate();
		m_pCheckCombo->RedrawWindow();
		m_pCheckCombo->BringWindowToTop();
	}
}
...
void CXListCtrl::DrawEdit(int nItem, int nSubItem, DWORD dwStyle)
{
...
LRESULT CXListCtrl::OnXEditKillFocus(WPARAM, LPARAM)
{
	...
	if (!pWnd)
		pWnd = GetOwner();
	if (pWnd && ::IsWindow(pWnd->m_hWnd))
		pWnd->SendMessage(WM_XLISTCTRL_EDIT_END, WPARAM(this), m_nEditItem);
